...
import com.steve.tickets.domain.dtos.GetTicketResponseDto;
...
public class TicketController {
    ...
    public ResponseEntity<GetTicketResponseDto> getTicket(@AuthenticationPrincipal Jwt jwt, @PathVariable UUID ticketId) {
        ...
        return ticketService.getTicketForUser(parseUserId(jwt), ticketId).map(ticketMapper::toGetTicketResponseDto).map(ResponseEntity::ok).orElse(ResponseEntity.notFound().build());
    }
